...
export const GridScan: React.FC<GridScanProps> = ({
  ...
  return (
    ...
    >
      {showPreview && (
        <div className="absolute right-3 bottom-3 w-55 h-33 rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-black text-white text-[12px] leading-[1.2] font-sans pointer-events-none">
          <video
            ref={videoRef}
            muted
            playsInline
          ...
          />
          <div className="absolute left-2 top-2 px-1.5 py-0.5 bg-black/50 rounded-[6px] backdrop-blur-1">
            {enableWebcam
              ? modelsReady
                ? uiFaceActive
                  ? "Face: tracking"
